...
The private repository uses the same `home/`, `setup/pre/`, and `setup/post/` layout. Running `./install.sh` in the public repository automatically:
...
```sh
DOTFILES_WORK_REPO="$HOME/private/work-dotfiles" ./install.sh
```

Set `DOTFILES_WORK_REPO` to an empty value to disable automatic discovery:

```sh
DOTFILES_WORK_REPO= ./install.sh
```
